...
export default async function CabinManagementPage() {
  ...
  try {
    ...
    console.log('Fetched cabins count:', cabins.length);
  } catch (error) {
    console.error('Error fetching cabins:', error);
    cabins = []; // Ensure it's always an array
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="w-80 bg-gradient-to-b from-slate-50 to-slate-100 border-r border-slate-200 flex flex-col">
        ...
        <nav className="flex-1 p-4">
          <div className="space-y-1">
            ...
            <a href="/admin/cabins" className="flex items-center gap-3 p-3 rounded-xl bg-blue-600 text-white shadow-sm font-medium">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
      ...
      {/* Main Content */}
      <CabinsClient initialCabins={cabins} />
    </div>
  );
}